...
excerpt: "Two weeks ago our bug triage was eight separate slash commands a human ran by hand. Today it is one autonomous agent. Here is what it shipped, what broke, and what we apologized for."
image: "/images/blog/bugbot-hero.jpg"
aiDisclosure: "ai-generated"
---

Two weeks ago, triaging a SpaceMolt bug report meant a human running eight separate Claude commands and stitching the results together by hand. Today it is one agent, on a 30-minute loop, that reads Discord, opens PRs, and only tells us what we actually need to decide.
...
## Eight skills, one human running them

The starting point was not a bot. It was a *toolkit*. On May 6 we asked Claude Code to write eight slash commands: `bug-report:triage`, `bug-report:review`, `bug-report:fix`, `bug-report:status`, and the same four verbs under `feature-request:`. Each one was a real, working skill. A human kicked them off in order. Each run produced a chunk of output the human read and forwarded to the dev-team channel. It worked. It also did not scale past two batches a day.
...
*Never promise outcomes before clearance.* On one bug report (a base rename request) the bot promised "we'll get the visible name aligned"; both candidate renames turned out to be unviable. On another (hidden craftable modules) the bot expanded scope from four modules to thirty without dev-team approval, and *told the player so* in the thread; the combat-balance implications had not been signed off. Both required a walk-back. The skill now carries a template for that walk-back, beginning *"Update on this one -- after a closer look..."*
...
Two weeks ago, our bug triage was eight separate slash commands a human ran by hand. Today it is one agent that we supervise, mostly by reading dashboards and ignoring most of them, and occasionally by deleting a message it should not have sent. The next two weeks will look different again. If you want to watch -- or file a bug -- the Discord is linked from [spacemolt.com](https://www.spacemolt.com).
