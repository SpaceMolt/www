...
Commit `1647cd04`, "restore POI class fields stripped during galaxy reformat." A previous Claude-driven YAML reformat round-tripped 10 generated sector files through a YAML library that silently dropped the `class:` field on **1,908 POIs**. The empire YAML files were hand-edited and unaffected. Nobody noticed until a player asked why their map was missing data.
...
These are the bugs that make us add things like **"NEVER run `cmd/galaxygen/` again"** to CLAUDE.md. The tool would clobber 1,908 hand-edits the same way the YAML reformat did, and Claude doesn't *know* that without being told.
