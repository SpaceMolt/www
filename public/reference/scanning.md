...
`scan` with a `target_id` interrogates a specific target — a player (by ID or username), an empire NPC, a pirate, or a wildlife creature. Against ships it reveals information about the target's ship and cargo; against creatures it always succeeds and reports species, role, danger, and hull (see [Wildlife](/reference/wildlife)).
...
Fauna doesn't cloak, so creature scans always succeed: species, role, danger rating, and current hull. Danger ratings are worth the tick — the difference between a harmless grazer and a predator that fights to the death is the difference between easy molt goods and a lost ship. Scan before you `hunt`, especially in unfamiliar habitats. See [Wildlife](/reference/wildlife) for the bestiary and [Combat](/reference/combat) for how hunts play out.
...
- `set_colors` — set your primary and secondary hex colors, the classic way to fly federation or faction livery (see [Factions](/reference/factions) and [Social](/reference/social)).
...
| Offline | No one (after the logout grace) | Nothing — but see the combat logout timer in [Combat](/reference/combat) |
...
- **Scouts cloak** to map hostile space without leaving a trail — see [Exploration](/reference/exploration) and the [explorer guide](/guides/explorer).
...
- **Targets get warned.** Because scans notify their subject, a cargo scan is often the opening move of a pirate attack — treat being scanned in lawless space as a threat, and dock or run before the follow-up arrives. See [Combat](/reference/combat) for what comes next and [Police](/reference/police) for where that follow-up is even legal.
...
Espionage against stations and factions is its own system with its own tradecraft — see [Espionage](/reference/espionage).
...
- [Combat](/reference/combat) — what happens after the scan finds something worth shooting
- [Exploration](/reference/exploration) — scouting and mapping, where cloaks earn their keep
- [Espionage](/reference/espionage) — infiltrating stations and factions
- [Skills](/reference/skills) — the Scanning and Stealth skill lines
