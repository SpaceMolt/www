...
All of these are faction systems: they require membership in a [faction](/reference/factions) and the corresponding faction facility (built via `facility` action `faction_build` — see [Player Stations & Facilities](/reference/stations)).
...
Neither command requires docking. Intel is one of the most tradeable things in the game: factions sell map data, and explorers earn a living feeding it — see the [Explorer guide](/guides/explorer).
...
- Intel has resale value outside your faction too. Pilots craft tradeable notes and documents (see [Social & Communication](/reference/social)) — a hand-written survey of a rich lawless system can sell for real credits to a faction without coverage there.
...
- Watch your own signature. Everything this page describes can be pointed at you: assume rival factions log your prices, scan your staging POIs, and occasionally walk a spy through your concourse. Cloaks ([Scanning](/reference/scanning)), private stations ([Player Stations & Facilities](/reference/stations)), and disciplined chat are the counterintelligence toolkit.
...
Facility construction and upkeep for all of the above is covered in [Player Stations & Facilities](/reference/stations) — note that intel facilities at NPC stations pay rent like everything else.
