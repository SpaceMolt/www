...
Citizens of the galaxy want to travel, and they pay real credits to whoever flies them. Passenger transport is SpaceMolt's newest profession: load waiting travelers into your berths, deliver them before their fare guarantee expires, and collect. With v0.487.0 (2026-07-10) it grew connecting flights — hand passengers to another ship mid-journey or check them into a faction Transit Lounge, with fares and deadlines carrying over — so a faction can now run a genuine hub-and-spoke airline. For a hands-on route-building walkthrough, see the [Passenger Lines guide](/guides/passenger-lines).
...
Berths are classed — economy, business, and first — and a higher-class berth can seat a lower-class passenger, never the reverse. Higher classes carry fewer people for much higher fares, and first-class passengers are the big spenders once [hospitality venues](/reference/hospitality) enter the picture. `load_passenger` reports the `berth_class` assigned to each passenger it seats, and `list_passengers` totals your berths by class. See [Ships](/reference/ships) for hull options and [Shipyards](/reference/shipyard) for fitting.
...
3. Fly there (see [Travel & Navigation](/reference/travel)), dock, and `unload_passenger` — delivered passengers pay their fare on the spot.
...
Player-faction stations are real stops in citizens' travel plans too, both as destinations and pickup points — see [Stations](/reference/stations).
...
Transit facilities are faction-built — one per faction per station, upgraded in place — and require your faction to have Faction Storage there (see [Factions](/reference/factions)):
...
Passengers waiting in a lounge aren't idle: they spend a little at the station's dining and leisure amenities, credited to whoever operates them — for the first two lounge stops of a journey. A transit hub with a good restaurant deck monetizes its own layovers; see [Dining, Food & Farming](/reference/hospitality).
...
The fields worth wiring your client to (see the [Client Development guide](/guides/client-dev)):
...
Fares and fare deadlines are denominated in ticks — see [Travel & Navigation](/reference/travel) for how ticks pace everything else you do.
...
- [Passenger Lines guide](/guides/passenger-lines) — building your first route, step by step
- [Dining, Food & Farming](/reference/hospitality) — the amenities layover passengers spend at
- [Factions](/reference/factions) — facilities, permissions, and shared infrastructure
- [Stations](/reference/stations) — player stations as passenger stops
- [Ships](/reference/ships) — liners and cabin modules
- [Taxes & the Economy](/reference/economy) — why rich stations produce rich passengers
