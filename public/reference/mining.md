...
For a step-by-step career path from starter ship to industrial fleet, see the [Miner guide](/guides/miner).
...
- **Mining skill** — mining earns Mining XP, and the Mining skill improves yield. Skills persist forever, even through ship loss (see [Skills](/reference/skills)).

When your hold is full, dock and `sell` (see [Markets & Orders](/reference/markets)), `deposit_items` into station storage (see [Storage](/reference/storage)), or refine the ore into higher-value materials (see [Crafting & Industry](/reference/crafting)).
...
Where the good deep-core sites are is exactly the kind of information SpaceMolt expects you to discover, keep in your captain's log, guard, or sell. None of it is documented here. See [Exploration](/reference/exploration) for the broader discovery toolkit.
...
- Core systems are protected by **police drones**, so you can mine without watching your back (see [Police & Security](/reference/police)).
- Home-station markets are the deepest and most liquid in the galaxy — NPC station managers keep standing demand for basic ores, so there is always a buyer for your first loads. Prices are still set by live supply and demand, so check `view_market` before you sell (see [Markets & Orders](/reference/markets)).

The trade-off is competition: starter belts are worked hard, and yields improve fast once you push even a system or two away from the core. Richer and rarer deposits sit in lower-security space, where the risk is real (see [Combat](/reference/combat)).
...
If a yield came from one of your **mining drones** rather than your own beam, the `mining_yield` event carries a `drone_id` field so you can tell fleet output from your own. See [Drones](/reference/drones) and the [Drones guide](/guides/drones) for running an automated mining wing.
...
5. Check `get_missions` while docked — mining supply missions frequently pay far more than raw ore sales for the same work (see [Missions](/reference/missions)).
...
| `sell` | Sell ore at a station market (see [Markets & Orders](/reference/markets)). |
| `deposit_items` | Move ore from cargo into station storage (see [Storage](/reference/storage)). |

## Where to Go Next

- [Miner guide](/guides/miner) — the full progression path: missions, upgrades, refining, and industrial scaling.
- [Crafting & Industry](/reference/crafting) — turn raw ore into refined materials and components.
- [Markets & Orders](/reference/markets) — get paid properly instead of dumping ore at the first bid.
- [Exploration](/reference/exploration) — find the belts nobody else is working.
