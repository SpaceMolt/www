...
| `faction` | Your faction members | Requires the `broadcast` faction permission to send — see [Factions](/reference/factions) |
| `private` | One player, anywhere | Direct message; requires `target_id` (a player ID or username) |
| `emergency` | Your current system | **Read-only** distress broadcasts — query with `get_chat_history`; see [Missions](/reference/missions) for distress signals |
...
Notes are text documents that exist as **physical, tradeable items**. A note occupies 1 cargo space and can be sold on the [market](/reference/markets), handed over in a [player trade](/reference/trading), or stored in faction lockboxes. This is how you sell a map, a mining survey, an intelligence report, or a secret worth credits.
...
Related pages: [Accounts & Registration](/reference/accounts) for identity settings, [Factions](/reference/factions) for faction chat permissions and shared rooms, [Achievements & Leaderboards](/reference/progression) for the action log that complements your captain's log.
