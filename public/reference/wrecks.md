...
When a ship dies — player, pirate, or creature — a wreck appears at the POI where it was destroyed. From the losing pilot's side of the ledger (see [Death, Cloning & Insurance](/reference/death)):
...
2. Travel to a station with a **salvage yard** and dock (see [Travel](/reference/travel) and [Stations](/reference/stations)).
...
Sell for immediate credits; scrap when you want inputs for [Crafting](/reference/crafting) or materials to move on the [Markets](/reference/markets). A high Salvaging skill tilts the math toward scrapping — better yields, including rare salvage that never appears in NPC shops.
...
- **Mind the neighborhood.** Looting is legal everywhere, but the people who made the wreck may still be around, and lawless space has no [Police](/reference/police) to object to them making another one out of you. A fast ship and an exit plan are part of the fit.

Skills matter here: **Salvaging** governs scrap yields and gates `scrap_wreck` (level 2+), and Engineering trims the CPU/power cost of fitting looted modules on the spot. See [Skills](/reference/skills) for progression.
...
Combined with a tow rig and a cargo-heavy hull, the skill supports a full non-combat career: you produce crafting inputs from other people's fights. Pirate hunters level it as a sideline (see the [pirate hunter guide](/guides/pirate-hunter)); dedicated vultures make it the main event.
...
- Pirates and mission targets leave wrecks too — bounty hunting plus systematic looting is a compounding income stream. See the [pirate hunter guide](/guides/pirate-hunter).
...
- Where the pickings are richest — active war fronts, pirate-heavy frontier belts — is exactly where new wrecks get made. Treat every salvage field as a potential ambush site and check [Scanning & Stealth](/reference/scanning) for reading who else is around before you commit to a slow tow.
...
- [Combat](/reference/combat) — how ships become wrecks, and how to win the fight over one
- [Death, Cloning & Insurance](/reference/death) — the other side of the ledger when the wreck is yours
- [Wildlife](/reference/wildlife) — carcass wrecks and molt goods
- [Skills](/reference/skills) — the Salvaging and Engineering skill lines
