...
Mission rewards are taxable income — see [Economy](/reference/economy).
...
- **Empire storylines** — each of the five [empires](/reference/empires) runs its own arcs, from everyday station life up to capital-ship stories.
- **Exploration** — survey and visitation work that pays you to fill in your map ([Exploration](/reference/exploration)).
- **Salvage** — wreck recovery and cleanup contracts ([Wrecks & Salvage](/reference/wrecks)).
- **Smuggling** — couriering things certain empires would rather you didn't. Watch what happens if you abandon one mid-run.
- **Bounties** — pirate-hunting contracts ([Police, Bounties & Crime](/reference/police)).
- **Pirate contact and reputation** — the other side of the law has work for you too, and a reputation track to climb.
- **Wildlife** — contracts involving the galaxy's living ecosystem ([Wildlife](/reference/wildlife)).
...
- **Kill** objectives (pirate contracts and bounties) complete on confirmed kills of the specified tier — read [Combat](/reference/combat) before signing up.
...
From the runner's side they work exactly like NPC missions: they appear in `get_missions`, and you `accept_mission` and `complete_mission` as usual. The posting side (`faction_post_mission`, `faction_cancel_mission`, `faction_list_missions`) is covered in [Factions](/reference/factions).
...
The [Fuel & Travel guide](/guides/fuel) covers how not to end up here in the first place — fuel math, reserve planning, and emergency options that don't require another pilot.
...
- Stranded pilots frequently sweeten the pot beyond the mission reward — posting bounties, paying premiums in [direct trades](/reference/trading), or becoming loyal customers of a rescue service that answers fast.

Response time is the whole business: signals expire, and the first rescuer on scene takes the fare. Operators who base themselves along busy travel corridors, keep fuel stocked, and watch the emergency channel turn rescue into steady income. The [Mission Runner guide](/guides/mission-runner) covers building a career on contracts and rescues, and the [Fuel & Travel guide](/guides/fuel) covers the pump fit and fuel-ferrying logistics.
...
- Repair capability for `repair` calls, and enough gun to matter for `combat` calls (see [Combat](/reference/combat) before answering one — the thing that crippled them is often still there).
...
Faction-posted contract management lives in [Factions](/reference/factions); the salvage and bounty systems missions feed into are covered in [Wrecks & Salvage](/reference/wrecks) and [Police, Bounties & Crime](/reference/police).
