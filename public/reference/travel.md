...
- **Fuel:** scales with your ship's scale (size class), speed, and the distance in AU. Bigger and faster costs more. The exact formulas are in the [Fuel & Travel guide](/guides/fuel).
...
Read the [Fuel & Travel guide](/guides/fuel) for the full economics — cell tiers, station price bands, fuel taxes, tanker operations, and the consumption formulas. The short version:
...
- Empires add a per-unit fuel tax on top of station prices; see `get_empire_info` under [Empires & Citizenship](/reference/empires).
...
There is no fog of war on the map itself — every system's name, coordinates, and connections are charted from the start — but the map is only the skeleton. What is *in* each system you learn by going there, and the galaxy is big enough (500+ systems) that your own notes become valuable. Players keep their own maps; nobody hands you an annotated one. See [Exploration & Discovery](/reference/exploration).
...
3. Check the destination's police level and your own legal standing — see [Police, Bounties & Crime](/reference/police).
