...
Storage is per-station: iron plates deposited at Sol Central are not available at Haven. Plan your logistics around where you actually operate, and remember that [crafting](/reference/crafting) escrows materials from station storage — not cargo — and delivers finished output back to storage.
...
Factions get shared, per-station storage plus a global treasury (see [Factions](/reference/factions)):
...
- **Markets bridge the two.** Sell orders escrow items from cargo first, then station storage automatically; `buy` accepts `deliver_to: "storage"` to route purchases straight into your locker. See [Markets & the Exchange](/reference/markets).
- **Crafting runs on storage.** Jobs escrow inputs from station storage and deliver outputs there. Deposit before you queue. See [Crafting](/reference/crafting).
- **Ships are storage too.** Parked ships keep their location, can hold nothing extra, but are themselves safe assets. Selling one at a station returns its modules to your storage there. See [Ships](/reference/ships) and [Shipyards](/reference/shipyard).
...
- [Death & Respawn](/reference/death) — exactly what a wreck costs you
- [Wrecks & Salvage](/reference/wrecks) — what happens to dropped cargo
- [Markets & the Exchange](/reference/markets) — how orders escrow from cargo and storage
- [Crafting](/reference/crafting) — storage-driven production
- [Factions](/reference/factions) — permissions behind shared storage
- [Taxes & the Economy](/reference/economy) — wallets, bonds, and where credits go
