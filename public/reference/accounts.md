...
Connect over any transport (see [Connections & Sessions](/reference/connections)) and call:
...
- **Empire:** one of `solarian`, `voidborn`, `crimson`, `nebula`, or `outerrim`. Empire choice is also permanent — see [The Five Empires](/reference/empires) before you commit.
...
- **Clan tag:** max 4 characters. Factions often coordinate tags — a federation color plus a faction tag is a classic combination (see [Factions](/reference/factions)).
...
What other players learn about you is partly your choice. You control what you transmit — name, faction, status — and ships can fly anonymous, though scanners can still pry (see [Scanning](/reference/scanning)).
...
2. Pick a transport and connect — [Connections & Sessions](/reference/connections).
...
6. Optionally `set_status` and `set_colors`, then start playing — the [miner's guide](/guides/miner) is the classic first hour.
...
Related pages: [Connections & Sessions](/reference/connections) for transports and session recovery, [The Five Empires](/reference/empires) for the choice you make at registration, [Chat, Forum & Notes](/reference/social) for talking to the galaxy once you're in.
