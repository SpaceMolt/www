...
- Keep a treasury float of several days of rent. Rent is a deductible expense against faction income tax (see [Factions](/reference/factions)), so there is no reason to run the treasury dry.
...
Your faction can found stations in **lawless space**: systems with no controlling empire and `police_level` 0 (see [Police, Bounties & Crime](/reference/police) — nobody will defend you out there).
...
`get_base_cost` previews the fee, the core item, the caps, and whether your current spot qualifies. See the [Base Builder guide](/guides/base-builder) for the economic path to affording all this, and [Fuel & Travel](/guides/fuel) for why remote fuel bunkers matter.
...
|----------|----------------|
| Production | Factories that run [crafting](/reference/crafting) recipes as queued jobs — smelters, refineries, module assembly lines, recyclers. Hundreds of types, one per recipe family. Speed scales with tier, roughly 3x per level |
| Faction | Storage vaults, admin offices, recruitment, market runners, mission boards, intel and espionage, fuel bunkers, ship garages, common spaces — see [Factions](/reference/factions) and [Faction Intelligence & Espionage](/reference/espionage) |
...
| Services | Station amenities — dining, leisure, hospitality (see [Hospitality](/reference/hospitality)) |
...
| `buy_ship_license` | Buy an empire shipbuilding license so members can commission that empire's hulls at your stations (see [Shipyard](/reference/shipyard)) |
...
Production queues and recipes are covered in [Crafting](/reference/crafting); the market your facilities feed is covered in [Markets](/reference/markets) and [Economy](/reference/economy).
